...
export const useMaterialTransformation111 = (materials: MaterialCollection, selectedMaterials: MaterialCollection) => {
    ...
    const processCutMaterial = (
        ...
        if (result.remainder && result.remainder.length > 0) {
            if (!cutParams.remainderCollection || cutParams.remainderCollection === cutParams.sourceCollection) {
                remainderCollection.addAfter(cutParams.material.uuid, result.remainder)
            } else {
        ...
        if (result.unusedPart) {
            if (cutParams.zeroOut) {
                if (result.unusedPart.quantity === 0 || result.unusedPart.amount === 0) {
                    sourceCollection.removeByUuid(cutParams.material.uuid)
                } else {
    ...
    watch(
        ...
        (materials) => {
            ...
            try {
                ...
                // Получаем противоположные стандарты для стыковки
                const oppositeStandards = standards.value.filterJoinedOpposite(firstMaterial.material_standard.id)
