...
export class Material extends BaseModel {
    ...
    hasSameBrands(material: Material): boolean {
        const thisBrands = this.material_standard.material_brands.pluck('id').sort();
        const otherBrands = material.material_standard.material_brands.pluck('id').sort();
        return _.isEqual(thisBrands, otherBrands);
    }
    ...
    get isAmountChanged(): boolean {
        return this.amount !== this.initial_amount
    }

    get isTotalWeightChanged(): boolean {
    ...
    get isJoined(): boolean {
        return this.material_standard.material_properties.pluck('id').includes(9)
    }

    get isPile(): boolean {
        return this.material_standard.material_type.getId() === MaterialTypes.PILE
    }

    get isAngularElement(): boolean {
        return this.material_standard.material_type.getId() === MaterialTypes.ANGULAR_ELEMENT
    }

    get isBeam(): boolean {
        return this.material_standard.material_type.getId() === MaterialTypes.I_BEAM
    }

    get isStraightSeamPipe(): boolean {
        return this.material_standard.material_type.getId() === MaterialTypes.STRAIGHT_SEAM_PIPE
    }
    ...
    resetByParts(materials: Material[]): void {

        const partsSum = _.sumBy(materials, (material: Material) => {
            return material.amount * material.quantity
        })

        if (this.isFixedQuantity) {
            this.amount = this.amount + (partsSum / this.quantity)
        } else {
            this.quantity += partsSum
            this.amount = 1
        }
    }
